...
\begin{definition}[Generators]\label{def:generators}
    A set $G \subseteq S$ is a generator of $S$ if and only if every element $x \in S$ can be expressed as a composition of elements from $G$.
\end{definition}
...
\begin{theorem}
    All atomics are elementals. For all siumatgwoons $S$, $A \subseteq E$.
\end{theorem}
\begin{proof}
    Let $a \in A$. $a$ clearly constitutes itself, so $a|a$. But for any decomposition, 
    Suppose $a$ is atomic but not elemental, which means it has a non-trivial decomposition, so $a=b*c$ for some $b,c \in S$. But then $b|a$ and $c|a$. Give any $x|a$ must imply $x=a$, so $b=a$ or $c=a$. But then we can apply the same argument, again, and we have 
\end{proof}
