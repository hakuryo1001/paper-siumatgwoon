...
\end{theorem}
\begin{proof}
    We have already proven that $\langle A \rangle \subseteq E$ for any $A\subseteq S$. Now we need to prove that $E \subseteq \langle A \rangle$.

    Let $x\in E$. Then $x$ is an elemental. And since $x$ is an elemental, it has no non-trivial decomposition. An object constituted by $a$ and has no non-trivial decomposition is an $a$-系 object, so $x \in \langle a \rangle$. Therefore, $E \subseteq \langle A \rangle$.
\end{proof}
