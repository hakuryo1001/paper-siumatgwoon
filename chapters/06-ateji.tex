...
\begin{definition}[Ateji]
    An object $s \in S$ is an ateji if $s$ is a emergent object $s= a_{x}$, i.e. $s\in\langle a \rangle$, where $x = x_{1} * \cdots * x_{n}$ (i.e. $x$ is a composite object)and $s= a_{x} ~ x_{1} * \cdots * x_{n}$.

    In other words, an object $s$ is an ateji if and only if it is an emergent object, and is synonymous with a composite object.

    An symbol $s \in S$ is an ateji if $s$ is a phonosemantic character $s= a_{x}$ where $x = x_{1} * \cdots * x_{n}$ (i.e. $x$ is a compound ideograph)and $s=a_{x} ~ x_{1} * \cdots * x_{n}$.
\end{definition}

\begin{definition}[Ateji]
    Ateji is a set of symbols that are used to represent the sounds of the Japanese language.
\end{definition}
